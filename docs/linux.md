...
Resolve CUDA installation. The CMake build and all six focused harness groups
have passed in WSL2 and native Arch/CachyOS Debug and Release configurations. A
native Arch/CachyOS bundle has also been installed and reported working in
Resolve. This is primary-machine evidence, not a general distro compatibility
claim or an exhaustive four-route/effects acceptance record. See the
accompanying validation record for measured runtime and driver requirements.
...
The source repository's `docs/building.md` describes the shared WSL/native
`linux-debug` and `linux-release` presets. Always use a fresh build cache when
moving between WSL and a native Linux installation.
