...
namespace {
    ...
    inline bool assign_glare_hash(Scanner::ScannerMediumRuntime& mediumRuntime) {
        ...
        mediumRuntime.effectsKey.glareHash = glareHash;
        return true;
    }
    ...
    bool validate_scanner_preflight_runtime(
        ...
        if (effectsKey.glareHash != expectedGlareHash) {
...
void JuicerProcessor::renderScannerFromDensity(const RenderContext& ctx, unsigned int threadCount) {
    ...
    Scanner::ScannerRuntimeEffectsKey effectsKey = scannerPreflight.effectsKey;
...
void JuicerProcessor::processImagesCUDA() {
    ...
    auto render_mode_from_scan_route = [](Spektrafilm::ScanRoute route) -> RenderMode {
        return Spektrafilm::scan_route_is_print(route) ? RenderMode::Print : RenderMode::NegativeOnly;
    };
    ...
    const RenderMode renderMode = render_mode_from_scan_route(scanRoute);
