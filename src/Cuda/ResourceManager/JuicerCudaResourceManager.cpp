...
namespace JuicerCuda {
    ...
    namespace ResourceManager {
        ...
        namespace {
            ...
            const char* failure_reason_class(const char* token) noexcept {
                ...
                if (value == "auto_no_optional_supported" ||
                    value == "capability_fallback_cuda_malloc" ||
            ...
            std::string trace_event_prefix(
                ...
                return std::string("event=") + trace_or_unknown(eventName) + " transaction_id=" + std::to_string(transaction.transactionId) + " snapshot_id=" + std::to_string(transaction.snapshot.snapshotId) + " trace_schema=" + std::to_string(transaction.snapshot.traceSchemaVersion) + " command=" + trace_or_unknown(commandName);
            }

            std::string trace_event_identity_prefix(
                const char* eventName,
                const SubmissionTransaction& transaction) {
                return std::string("event=") + trace_or_unknown(eventName) + " transaction_id=" + std::to_string(transaction.transactionId) + " snapshot_id=" + std::to_string(transaction.snapshot.snapshotId) + " trace_schema=" + std::to_string(transaction.snapshot.traceSchemaVersion);
            }
            ...
            struct ScratchBucketKeyHash {
                std::size_t operator()(const ScratchBucketKey& key) const noexcept {
                    ...
                    return (((hW ^ (hH + 0x9e3779b9u + (hW << 6u) + (hW >> 2u))) ^ (hStep + 0x9e3779b9u + (hW << 6u) + (hW >> 2u))) ^ (hWork + 0x9e3779b9u + (hW << 6u) + (hW >> 2u))) ^ (hLarge + 0x9e3779b9u + (hW << 6u) + (hW >> 2u));
                }
            };
            ...
            void trace_5x_env_overrides(const FiveXEnvOverrides& overrides, const ResourceManagerConfigEffective& cfg) {
                ...
                const std::string msg = std::string("event=5x_env_overrides") + " profile_enabled=" + std::to_string(overrides.profileEnabled ? 1 : 0) + " profile_disabled=" + std::to_string(overrides.profileDisabled ? 1 : 0) + " manager_soft_target_bytes=" + std::to_string(static_cast<unsigned long long>(cfg.managerSoftTargetBytes)) + " manager_reserve_bytes=" + std::to_string(static_cast<unsigned long long>(cfg.managerReserveBytes)) + " keep_hot_ms=" + std::to_string(static_cast<unsigned long long>(cfg.keepHotMs)) + " admission_churn_window_ms=" + std::to_string(static_cast<unsigned long long>(cfg.admissionChurnWindowMs)) + " admission_churn_enter_one_hit_rate_pct=" + std::to_string(static_cast<unsigned long long>(cfg.admissionChurnEnterOneHitRatePct)) + " admission_churn_exit_one_hit_rate_pct=" + std::to_string(static_cast<unsigned long long>(cfg.admissionChurnExitOneHitRatePct)) + " admission_churn_probation_hit_bonus=" + std::to_string(static_cast<unsigned long long>(cfg.admissionChurnProbationHitBonus)) + " large_entry_readmit_cooldown_ms=" + std::to_string(static_cast<unsigned long long>(cfg.largeEntryReadmitCooldownMs)) + " large_entry_ghost_hits_for_readmit=" + std::to_string(static_cast<unsigned long long>(cfg.largeEntryGhostHitsForReadmit)) + " burst_debt_half_life_ms=" + std::to_string(static_cast<unsigned long long>(cfg.burstDebtHalfLifeMs)) + " max_burst_debt_pct=" + std::to_string(static_cast<unsigned long long>(cfg.maxBurstDebtPct)) + " cancel_superseded_builders=" + std::to_string(cfg.cancelSupersededBuilders ? 1 : 0);
                JTRACE("MSCFG", msg);
#endif
            }
            ...
            std::string trace_resolved_policy_fields(
                ...
                const ResolvedPressurePolicy& policy = transaction.resolvedPressurePolicy;
                return std::string(" policy_source=") + to_cstr(policy.policySource) + " policy_device_id=" + std::to_string(policy.policyDeviceId) + " soft_target_bytes=" + std::to_string(static_cast<unsigned long long>(softTargetBytes)) + " reserve_bytes=" + std::to_string(static_cast<unsigned long long>(reserveBytes)) + " reader_matches_submission_policy=" + std::to_string(trace_policy_match_u32(transaction, softTargetBytes, reserveBytes));
            }
            ...
            std::string trace_headroom_identity_fields(
                ...
                return std::string(" submission_device_id=") + std::to_string(submissionDeviceId) + " sampled_device_id=" + std::to_string(headroom.sampledDeviceId) + " sample_matches_submission_device=" + std::to_string(sampleMatchesSubmissionDevice ? 1u : 0u) + " sample_success=" + std::to_string(headroom.sampleSuccess ? 1u : 0u);
            }
#endif
        ...
        struct ScratchPolicyDecisionTrace {
            const char* commandName = nullptr;
            const char* result = nullptr;
            const ScratchPolicySnapshot* snapshot = nullptr;
        ...
        void trace_scratch_policy_decision(
            const SubmissionTransaction& transaction,
            const ScratchPolicyDecisionTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const ScratchPolicySnapshot& snapshot = *trace.snapshot;
            ...
            const std::size_t effectiveTempCap = effective_temp_scratch_bytes_cap(trace.requestBytes);

            const std::string msg = trace_event_prefix("scratch_policy", transaction, trace.commandName) + " result=" + trace_or_unknown(trace.result) + " work_class=" + to_cstr(snapshot.bucketKey.workClass) + trace_device_context_fields(transaction) + " request_bytes=" + std::to_string(static_cast<unsigned long long>(trace.requestBytes)) + " in_flight_bytes=" + std::to_string(static_cast<unsigned long long>(snapshot.inFlightBytes)) + " in_flight_sets=" + std::to_string(snapshot.inFlightSets) + " bucket_w=" + std::to_string(snapshot.bucketKey.widthBucket) + " bucket_h=" + std::to_string(snapshot.bucketKey.heightBucket) + " bucket_step_px=" + std::to_string(snapshot.bucketKey.bucketStepPx) + " large_frame=" + std::to_string(snapshot.bucketKey.largeFrame ? 1 : 0) + " bucket_attempts=" + std::to_string(snapshot.bucketAttempts) + " bucket_exhausted=" + std::to_string(snapshot.bucketExhausted) + " bucket_alloc_growth=" + std::to_string(snapshot.bucketAllocGrowth) + " bucket_reuse=" + std::to_string(snapshot.bucketReuse) + " bucket_starvation_latched=" + std::to_string(snapshot.starvationLatched ? 1 : 0) + " scratch_churn_ratio_milli=" + std::to_string(churnRatioMilli) + " quarantine_bytes=" + std::to_string(static_cast<unsigned long long>(snapshot.quarantineBytes)) + " quarantine_entries=" + std::to_string(snapshot.quarantineEntries) + " wait_ms=" + std::to_string(trace.waitMs) + " max_temp_sets=" + std::to_string(kMaxTempScratchSets) + " max_temp_bytes=" + std::to_string(static_cast<unsigned long long>(kMaxTempScratchBytes)) + " effective_max_temp_bytes=" + std::to_string(static_cast<unsigned long long>(effectiveTempCap)) + " quarantine_max_bytes=" + std::to_string(static_cast<unsigned long long>(kLargeFrameQuarantineMaxBytes)) + " quarantine_max_entries=" + std::to_string(static_cast<unsigned long long>(kLargeFrameQuarantineMaxEntries));
            JTRACE("MSACQ", msg);
#endif
        }

        struct ReservationDecisionTrace {
            const char* commandName = nullptr;
            const char* reason = nullptr;
            const ReservationDecision* decision = nullptr;
        ...
        void trace_transient_reservation_decision(
            const SubmissionTransaction& transaction,
            const ReservationDecisionTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const ReservationDecision& decision = *trace.decision;
            const std::string msg = trace_event_prefix("transient_reservation", transaction, trace.commandName) + " kind=" + to_cstr(ReservationKind::TransientNonManager) + trace_resolved_policy_fields(transaction) + trace_device_context_fields(transaction) + " request_bytes=" + std::to_string(static_cast<unsigned long long>(trace.requestBytes)) + " bytes_in_flight=" + std::to_string(static_cast<unsigned long long>(trace.bytesInFlight)) + " cap_bytes=" + std::to_string(static_cast<unsigned long long>(trace.capBytes)) + " threshold_bytes=" + std::to_string(static_cast<unsigned long long>(trace.thresholdBytes)) + " granted=" + std::to_string(decision.granted ? 1 : 0) + " should_wait=" + std::to_string(decision.shouldWait ? 1 : 0) + " wait_ms=" + std::to_string(trace.waitMs) + " critical_current_frame=" + std::to_string(trace.criticalCurrentFrame ? 1 : 0) + " decision_reason=" + trace_or_unspecified(decision.reason) + " decision_reason_class=" + trace_reason_class_or_invalid(decision.reason) + " reason=" + trace_or_unspecified(trace.reason) + trace_reason_class_field_if_known("reason_class", trace.reason);
            JTRACE("MSTRS", msg);
#endif
        }
        ...
        void trace_upload_reservation_decision(
            ...
            const ReservationDecision& decision = *trace.decision;
            const std::string msg = trace_event_prefix("upload_reservation", transaction, trace.commandName) + " kind=" + to_cstr(ReservationKind::UploadCopy) + " instance_token=" + std::to_string(static_cast<unsigned long long>(trace.instanceToken)) + trace_resolved_policy_fields(transaction) + trace_device_context_fields(transaction) + " request_bytes=" + std::to_string(static_cast<unsigned long long>(trace.requestBytes)) + " bytes_in_flight=" + std::to_string(static_cast<unsigned long long>(trace.bytesInFlight)) + " cap_bytes=" + std::to_string(static_cast<unsigned long long>(trace.capBytes)) + " threshold_bytes=" + std::to_string(static_cast<unsigned long long>(trace.thresholdBytes)) + " shared_tokens=" + std::to_string(static_cast<unsigned long long>(trace.sharedTokens)) + " critical_tokens=" + std::to_string(static_cast<unsigned long long>(trace.criticalTokens)) + " granted=" + std::to_string(decision.granted ? 1 : 0) + " should_wait=" + std::to_string(decision.shouldWait ? 1 : 0) + " wait_ms=" + std::to_string(trace.waitMs) + " critical_current_frame=" + std::to_string(trace.criticalCurrentFrame ? 1 : 0) + " decision_reason=" + trace_or_unspecified(decision.reason) + " decision_reason_class=" + trace_reason_class_or_invalid(decision.reason) + " reason=" + trace_or_unspecified(trace.reason) + trace_reason_class_field_if_known("reason_class", trace.reason);
            JTRACE("MSUPL", msg);
#endif
        }
        ...
        void trace_builder_reservation_decision(
            ...
            const ReservationDecision& decision = *trace.decision;
            const std::string msg = trace_event_prefix("builder_reservation", transaction, trace.commandName) + " kind=" + to_cstr(ReservationKind::BuilderWork) + " tier=" + to_cstr(tier) + " instance_token=" + std::to_string(static_cast<unsigned long long>(trace.instanceToken)) + trace_device_context_fields(transaction) + " request_bytes=" + std::to_string(static_cast<unsigned long long>(trace.requestBytes)) + " bytes_in_flight=" + std::to_string(static_cast<unsigned long long>(trace.bytesInFlight)) + " cap_bytes=" + std::to_string(static_cast<unsigned long long>(trace.capBytes)) + " threshold_bytes=" + std::to_string(static_cast<unsigned long long>(trace.thresholdBytes)) + " shared_tokens=" + std::to_string(static_cast<unsigned long long>(trace.sharedTokens)) + " critical_tokens=" + std::to_string(static_cast<unsigned long long>(trace.criticalTokens)) + " granted=" + std::to_string(decision.granted ? 1 : 0) + " should_wait=" + std::to_string(decision.shouldWait ? 1 : 0) + " wait_ms=" + std::to_string(trace.waitMs) + " critical_current_frame=" + std::to_string(trace.criticalCurrentFrame ? 1 : 0) + " decision_reason=" + trace_or_unspecified(decision.reason) + " decision_reason_class=" + trace_reason_class_or_invalid(decision.reason) + " reason=" + trace_or_unspecified(trace.reason) + trace_reason_class_field_if_known("reason_class", trace.reason);
            JTRACE("MSBPR", msg);
#endif
        }

        struct TierCircuitEventTrace {
            const char* commandName = nullptr;
            const char* reason = nullptr;
            TierCircuitState previousState = TierCircuitState::Closed;
            TierCircuitState state = TierCircuitState::Closed;
            ResourceTier tier = ResourceTier::Immutable;
            std::uint64_t openRemainingMs = 0;
            std::uint32_t windowErrors = 0;
        ...
        void trace_tier_circuit_event(
            const SubmissionTransaction& transaction,
            const TierCircuitEventTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const std::string msg = trace_event_prefix("tier_circuit", transaction, trace.commandName) + " tier=" + to_cstr(trace.tier) + " prev_state=" + to_cstr(trace.previousState) + " state=" + to_cstr(trace.state) + " transition=" + std::to_string(trace.transition ? 1 : 0) + " allowed=" + std::to_string(trace.allowed ? 1 : 0) + " probe=" + std::to_string(trace.probe ? 1 : 0) + " window_errors=" + std::to_string(trace.windowErrors) + " threshold=" + std::to_string(trace.threshold) + " window_ms=" + std::to_string(trace.windowMs) + " open_ms=" + std::to_string(trace.openMs) + " open_remaining_ms=" + std::to_string(static_cast<unsigned long long>(trace.openRemainingMs)) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(trace.reason);
            JTRACE("MSCB", msg);
#endif
        }

        struct TierBudgetEventTrace {
            const char* commandName = nullptr;
            const char* reason = nullptr;
        ...
        void trace_tier_budget_event(
            const SubmissionTransaction& transaction,
            const TierBudgetEventTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }
        ...
        struct PressureGateReaderTrace {
            const char* commandName = nullptr;
            const char* path = nullptr;
            const char* reason = nullptr;
            const ScratchRequestDescriptor* scratchRequest = nullptr;
            PressureLane lane = PressureLane::Builder;
            std::size_t requestBytes = 0;
            bool criticalCurrentFrame = false;
        };
        ...
        void trace_pressure_gate_reader(
            const SubmissionTransaction& transaction,
            const PressureGateReaderTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const std::string msg = trace_event_prefix("pressure_gate_reader", transaction, trace.commandName) + " lane=" + to_cstr(trace.lane) + " request_bytes=" + std::to_string(static_cast<unsigned long long>(trace.requestBytes)) + " critical_current_frame=" + std::to_string(trace.criticalCurrentFrame ? 1 : 0) + " pressure_policy_enabled=" + std::to_string(pressure_policy_enabled(transaction.resolvedPressurePolicy) ? 1 : 0) + trace_resolved_policy_fields(transaction) + " request_active=" + std::to_string(trace.scratchRequest ? 1 : 0) + " request_generation=" + std::to_string(static_cast<unsigned long long>(trace.scratchRequest ? trace.scratchRequest->generation : 0)) + " path=" + trace_or_unspecified(trace.path) + " reason=" + trace_or_unspecified(trace.reason);
            JTRACE("MSPGR", msg);
#endif
        }

        struct ScratchSheddingEventTrace {
            const char* commandName = nullptr;
            const char* actionName = nullptr;
            const char* reason = nullptr;
            std::uint64_t candidateLiveBytesBefore = 0;
            std::uint64_t candidateLiveBytesAfter = 0;
            std::uint64_t policyLiveBytesBefore = 0;
            std::uint64_t policyLiveBytesAfter = 0;
            ScratchCheckpointInvocation invocation = ScratchCheckpointInvocation::OuterPhaseCheckpoint;
            bool orphanedSharedTmpRetired = false;
            bool partialFailure = false;
        };
        ...
        void trace_scratch_shedding_event(
            const SubmissionTransaction& transaction,
            const ScratchSheddingEventTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const std::string msg = trace_event_prefix("scratch_shedding", transaction, trace.commandName) + " invocation=" + to_cstr(trace.invocation) + " action=" + trace_or_unspecified(trace.actionName) + " candidate_live_bytes_before=" + std::to_string(static_cast<unsigned long long>(trace.candidateLiveBytesBefore)) + " candidate_live_bytes_after=" + std::to_string(static_cast<unsigned long long>(trace.candidateLiveBytesAfter)) + " policy_live_retained_bytes_before=" + std::to_string(static_cast<unsigned long long>(trace.policyLiveBytesBefore)) + " policy_live_retained_bytes_after=" + std::to_string(static_cast<unsigned long long>(trace.policyLiveBytesAfter)) + " orphaned_shared_tmp_retired=" + std::to_string(trace.orphanedSharedTmpRetired ? 1 : 0) + " partial_failure=" + std::to_string(trace.partialFailure ? 1 : 0) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(trace.reason);
            JTRACE("MSSSH", msg);
#endif
        }

        struct EffectiveReserveEventTrace {
            const char* commandName = nullptr;
            const char* reason = nullptr;
            std::uint64_t reserveBaseBytes = 0;
            std::uint64_t reserveBeforeBytes = 0;
            std::uint64_t reserveTargetBytes = 0;
            std::uint64_t reserveAfterBytes = 0;
            std::uint64_t transientNonManagerBytes = 0;
            bool updated = false;
        };
        ...
        void trace_effective_reserve_event(
            const SubmissionTransaction& transaction,
            const EffectiveReserveEventTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }
            ...
            const std::string msg = trace_event_prefix("effective_reserve", transaction, trace.commandName) + trace_resolved_policy_fields(transaction, transaction.resolvedPressurePolicy.softTargetBytes, trace.reserveBaseBytes) + " reserve_base_bytes=" + std::to_string(static_cast<unsigned long long>(trace.reserveBaseBytes)) + " reserve_before_bytes=" + std::to_string(static_cast<unsigned long long>(trace.reserveBeforeBytes)) + " reserve_target_bytes=" + std::to_string(static_cast<unsigned long long>(trace.reserveTargetBytes)) + " reserve_after_bytes=" + std::to_string(static_cast<unsigned long long>(trace.reserveAfterBytes)) + " transient_non_manager_bytes=" + std::to_string(static_cast<unsigned long long>(trace.transientNonManagerBytes)) + " reserve_safety_margin_bytes=" + std::to_string(static_cast<unsigned long long>(cfg.reserveSafetyMarginBytes)) + " reserve_step_up_bytes=" + std::to_string(static_cast<unsigned long long>(cfg.reserveAdaptUpStepBytes)) + " reserve_step_down_bytes=" + std::to_string(static_cast<unsigned long long>(cfg.reserveAdaptDownStepBytes)) + " updated=" + std::to_string(trace.updated ? 1 : 0) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(trace.reason);
            JTRACE("MSRSV", msg);
#endif
        }

        struct OpportunisticFreezeEventTrace {
            const char* commandName = nullptr;
            const char* reason = nullptr;
            std::uint64_t effectiveHeadroomBytes = 0;
            std::uint64_t effectiveReserveBytes = 0;
        ...
        void trace_opportunistic_freeze_event(
            const SubmissionTransaction& transaction,
            const OpportunisticFreezeEventTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const std::string msg = trace_event_prefix("opportunistic_freeze", transaction, trace.commandName) + " state=" + to_cstr(trace.pressureState) + " effective_headroom_bytes=" + std::to_string(static_cast<unsigned long long>(trace.effectiveHeadroomBytes)) + " effective_reserve_bytes=" + std::to_string(static_cast<unsigned long long>(trace.effectiveReserveBytes)) + " frozen=" + std::to_string(trace.frozen ? 1 : 0) + " allowed=" + std::to_string(trace.allowed ? 1 : 0) + " critical_current_frame=" + std::to_string(trace.criticalCurrentFrame ? 1 : 0) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(trace.reason);
            JTRACE("MSFRZ", msg);
#endif
        }
        ...
        void trace_active_burst_event(
            ...
            const std::string msg = trace_event_prefix("active_burst", transaction, commandName) + " considered=" + std::to_string(burst.considered ? 1 : 0) + " active=" + std::to_string(burst.active ? 1 : 0) + " allowed=" + std::to_string(burst.allowed ? 1 : 0) + " entered=" + std::to_string(burst.entered ? 1 : 0) + " exited=" + std::to_string(burst.exited ? 1 : 0) + " cap_hit=" + std::to_string(burst.capHit ? 1 : 0) + " over_target_bytes=" + std::to_string(static_cast<unsigned long long>(burst.overTargetBytes)) + " cap_bytes=" + std::to_string(static_cast<unsigned long long>(burst.capBytes)) + " elapsed_ms=" + std::to_string(static_cast<unsigned long long>(burst.elapsedMs)) + " critical_current_frame=" + std::to_string(criticalCurrentFrame ? 1 : 0) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(reason);
            JTRACE("MSBURST", msg);
#endif
        }
        ...
        void trace_transient_non_manager_sample(
            ...
            const std::string msg = trace_event_prefix("transient_non_manager_sample", transaction, commandName) + trace_device_context_fields(transaction) + trace_resolved_policy_fields(transaction, checkpoint.input.softTargetBytes, checkpoint.input.reserveBytes) + " transient_non_manager_bytes=" + std::to_string(static_cast<unsigned long long>(checkpoint.input.transientNonManagerBytes)) + " pending_growth_bytes=" + std::to_string(static_cast<unsigned long long>(pendingGrowthBytes)) + " pressure_total_bytes=" + std::to_string(static_cast<unsigned long long>(pressure_total_bytes(checkpoint.input))) + " reason=" + trace_or_unspecified(reason);
            JTRACE("MSTRN", msg);
#endif
        }

        struct EmergencyShedActionTrace {
            const char* commandName = nullptr;
            const char* reason = nullptr;
            std::uint64_t uploadBytesInFlight = 0;
            std::uint64_t uploadCapBytes = 0;
            std::size_t requestBytes = 0;
            PressureLane lane = PressureLane::Builder;
        ...
        void trace_emergency_shed_action(
            const SubmissionTransaction& transaction,
            const EmergencyShedActionTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const std::string msg = trace_event_prefix("emergency_shed", transaction, trace.commandName) + " lane=" + to_cstr(trace.lane) + " state=" + to_cstr(trace.state) + " request_bytes=" + std::to_string(static_cast<unsigned long long>(trace.requestBytes)) + " critical_current_frame=" + std::to_string(trace.criticalCurrentFrame ? 1 : 0) + " allowed=" + std::to_string(trace.allowed ? 1 : 0) + " upload_bytes_in_flight=" + std::to_string(static_cast<unsigned long long>(trace.uploadBytesInFlight)) + " upload_cap_bytes=" + std::to_string(static_cast<unsigned long long>(trace.uploadCapBytes)) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(trace.reason);
            JTRACE("MSEMS", msg);
#endif
        }

        struct LaneWaitEventTrace {
            const char* commandName = nullptr;
        ...
        void trace_lane_wait_event(
            const SubmissionTransaction& transaction,
            const LaneWaitEventTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const std::string msg = trace_event_prefix("lane_wait", transaction, trace.commandName) + " lane=" + to_cstr(trace.lane) + " critical_current_frame=" + std::to_string(trace.criticalCurrentFrame ? 1 : 0) + " wait_ms=" + std::to_string(trace.waitMs) + " outcome=" + trace_or_unknown(trace.outcome) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(trace.reason);
            JTRACE("MSFAIR", msg);
#endif
        }

        struct CopyComputeGuardTrace {
            const char* commandName = nullptr;
            const char* reason = nullptr;
            std::uint64_t uploadBytesInFlight = 0;
            std::uint64_t uploadCapBytes = 0;
        ...
        void trace_copy_compute_guard(
            const SubmissionTransaction& transaction,
            const CopyComputeGuardTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const std::string msg = trace_event_prefix("copy_compute_guard", transaction, trace.commandName) + " state=" + to_cstr(trace.state) + " upload_bytes_in_flight=" + std::to_string(static_cast<unsigned long long>(trace.uploadBytesInFlight)) + " upload_cap_bytes=" + std::to_string(static_cast<unsigned long long>(trace.uploadCapBytes)) + " critical_current_frame=" + std::to_string(trace.criticalCurrentFrame ? 1 : 0) + " allowed=" + std::to_string(trace.allowed ? 1 : 0) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(trace.reason);
            JTRACE("MSCOPY", msg);
#endif
        }
        ...
        AdmissionChurnSnapshot sample_admission_churn_state(
            const SubmissionTransaction& transaction,
            const ResourceManagerConfigEffective& cfg,
            const AdmissionChurnSampleInput& input) {
            ...
            if (input.observedProbationHits == 0) {
                contextState.windowOneHitSamples += 1;
            }
            std::uint32_t& digestHits = contextState.windowDigestHits[input.entryDigest];
        ...
        void trace_keep_hot_surface(
            ...
            const std::string msg = trace_event_identity_prefix("keep_hot_surface", transaction) + trace_device_context_fields(transaction) + " enabled=" + std::to_string(cfg.keepHotMs > 0 ? 1 : 0) + " keep_hot_ms=" + std::to_string(static_cast<unsigned long long>(cfg.keepHotMs)) + " reason=" + trace_or_unspecified(reason) + trace_reason_class_field_if_known("reason_class", reason);
            JTRACE("MSHOT", msg);
            telemetry_counter_add(global_state().keepHotSurfaceTraceEvents, 1);
#endif
        }

        struct KeepHotDecisionTrace {
            const char* commandName = nullptr;
            const char* reason = nullptr;
            std::uint64_t keepHotMs = 0;
            std::uint64_t bypassEvents = 0;
            std::uint64_t forcedEvictEvents = 0;
        };

        void trace_keep_hot_decision(
            const SubmissionTransaction& transaction,
            const KeepHotDecisionTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const std::string msg = trace_event_prefix("keep_hot_decision", transaction, trace.commandName) + " keep_hot_ms=" + std::to_string(static_cast<unsigned long long>(trace.keepHotMs)) + " bypass_events=" + std::to_string(static_cast<unsigned long long>(trace.bypassEvents)) + " forced_evict_events=" + std::to_string(static_cast<unsigned long long>(trace.forcedEvictEvents)) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(trace.reason) + trace_reason_class_field_if_known("reason_class", trace.reason);
            JTRACE("MSHOT", msg);
#endif
        }
        ...
        void trace_burst_debt_surface(
            ...
            const std::string msg = trace_event_identity_prefix("burst_debt_surface", transaction) + trace_device_context_fields(transaction) + " enabled=" + std::to_string(enabled ? 1 : 0) + " burst_debt_half_life_ms=" + std::to_string(static_cast<unsigned long long>(cfg.burstDebtHalfLifeMs)) + " max_burst_debt_pct=" + std::to_string(static_cast<unsigned long long>(cfg.maxBurstDebtPct)) + " reason=" + trace_or_unspecified(reason) + trace_reason_class_field_if_known("reason_class", reason);
            JTRACE("MSBDE", msg);
        ...
        void trace_burst_debt_decision(
            ...
            const std::string msg = trace_event_prefix("burst_debt_decision", transaction, commandName) + " lane=" + to_cstr(lane) + " pressure_state=" + to_cstr(pressureState) + " request_bytes=" + std::to_string(static_cast<unsigned long long>(requestBytes)) + " critical_current_frame=" + std::to_string(criticalCurrentFrame ? 1 : 0) + " enabled=" + std::to_string(decision.enabled ? 1 : 0) + " sampled=" + std::to_string(decision.sampled ? 1 : 0) + " burst_consumed=" + std::to_string(decision.burstConsumed ? 1 : 0) + " throttled=" + std::to_string(decision.throttled ? 1 : 0) + " debt_before_pct=" + std::to_string(decision.debtBeforePct) + " debt_after_pct=" + std::to_string(decision.debtAfterPct) + " debt_increment_pct=" + std::to_string(decision.debtIncrementPct) + " burst_debt_half_life_ms=" + std::to_string(static_cast<unsigned long long>(cfg.burstDebtHalfLifeMs)) + " max_burst_debt_pct=" + std::to_string(static_cast<unsigned long long>(cfg.maxBurstDebtPct)) + " instance_token=" + std::to_string(static_cast<unsigned long long>(transaction.snapshot.instanceToken.value)) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(decision.reason) + " reason_class=" + trace_reason_class_or_invalid(decision.reason);
            JTRACE("MSBDE", msg);
#endif
        }
        ...
        void trace_superseded_builder_cancel_surface(
            ...
            const std::string msg = trace_event_identity_prefix("superseded_builder_cancel_surface", transaction) + trace_device_context_fields(transaction) + " enabled=" + std::to_string(cfg.cancelSupersededBuilders ? 1 : 0) + " reason=" + trace_or_unspecified(reason) + trace_reason_class_field_if_known("reason_class", reason);
            JTRACE("MSCNL", msg);
            telemetry_counter_add(global_state().supersededBuilderCancelSurfaceTraceEvents, 1);
#endif
        }

        struct SupersededBuilderCancelTrace {
            const char* commandName = nullptr;
            const SupersededBuilderCancelInput* input = nullptr;
            const SupersededBuilderCancelDecision* decision = nullptr;
            std::uint64_t requestBytes = 0;
        ...
        void trace_superseded_builder_cancel_decision(
            const SubmissionTransaction& transaction,
            const SupersededBuilderCancelTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }
            const SupersededBuilderCancelInput& input = *trace.input;
            const SupersededBuilderCancelDecision& decision = *trace.decision;
            const std::uint64_t savedBytes = decision.cancel ? trace.requestBytes : 0;
            const std::string msg = trace_event_prefix("superseded_builder_cancel_decision", transaction, trace.commandName) + " enabled=" + std::to_string(input.enabled ? 1 : 0) + " superseded=" + std::to_string(input.superseded ? 1 : 0) + " critical_current_frame=" + std::to_string(trace.criticalCurrentFrame ? 1 : 0) + " cancel=" + std::to_string(decision.cancel ? 1 : 0) + " request_bytes=" + std::to_string(static_cast<unsigned long long>(trace.requestBytes)) + " saved_bytes=" + std::to_string(static_cast<unsigned long long>(savedBytes)) + " latest_snapshot_id=" + std::to_string(static_cast<unsigned long long>(trace.latestSnapshotId)) + " instance_token=" + std::to_string(static_cast<unsigned long long>(transaction.snapshot.instanceToken.value)) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(decision.reason) + " reason_class=" + trace_reason_class_or_invalid(decision.reason);
            JTRACE("MSCNL", msg);
#endif
        }
        ...
        struct LargeEntryReadmitTrace {
            const char* commandName = nullptr;
            const LargeEntryReadmitDecision* decision = nullptr;
            std::uint64_t entryDigest = 0;
            std::uint64_t requestBytes = 0;
            std::uint64_t thresholdBytes = 0;
        };
        ...
        void trace_large_entry_readmit_decision(
            const SubmissionTransaction& transaction,
            const LargeEntryReadmitTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const LargeEntryReadmitDecision& decision = *trace.decision;
            const std::string msg = trace_event_prefix("large_entry_readmit", transaction, trace.commandName) + " entry_digest=" + std::to_string(static_cast<unsigned long long>(trace.entryDigest)) + " request_bytes=" + std::to_string(static_cast<unsigned long long>(trace.requestBytes)) + " threshold_bytes=" + std::to_string(static_cast<unsigned long long>(trace.thresholdBytes)) + " enabled=" + std::to_string(decision.enabled ? 1 : 0) + " candidate=" + std::to_string(decision.candidate ? 1 : 0) + " critical_current_frame=" + std::to_string(decision.criticalCurrentFrame ? 1 : 0) + " had_history=" + std::to_string(decision.hadHistory ? 1 : 0) + " in_cooldown=" + std::to_string(decision.inCooldown ? 1 : 0) + " blocked=" + std::to_string(decision.blocked ? 1 : 0) + " ghost_bypass=" + std::to_string(decision.ghostBypass ? 1 : 0) + " age_ms=" + std::to_string(static_cast<unsigned long long>(decision.ageMs)) + " cooldown_ms=" + std::to_string(static_cast<unsigned long long>(decision.cooldownMs)) + " ghost_hits_required=" + std::to_string(decision.ghostHitsRequired) + " observed_ghost_hits=" + std::to_string(decision.observedGhostHits) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(decision.reason) + " reason_class=" + trace_reason_class_or_invalid(decision.reason);
            JTRACE("MSTHR", msg);
#endif
        }

        struct CacheAdmissionDecisionTrace {
            const char* commandName = nullptr;
            const char* reason = nullptr;
            const CacheAdmissionDecision* decision = nullptr;
            const AdmissionChurnSnapshot* churnSnapshot = nullptr;
            std::uint64_t requestBytes = 0;
            std::uint64_t entryDigest = 0;
            std::uint32_t observedProbationHits = 0;
            bool criticalCurrentFrame = false;
        ...
        void trace_cache_admission_decision(
            const SubmissionTransaction& transaction,
            const CacheAdmissionDecisionTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const CacheAdmissionDecision& decision = *trace.decision;
            const AdmissionChurnSnapshot& churnSnapshot = *trace.churnSnapshot;
            const std::string msg = trace_event_prefix("cache_admission", transaction, trace.commandName) + " class=" + to_cstr(decision.admissionClass) + " allow_durable=" + std::to_string(decision.allowDurableAdmission ? 1 : 0) + " probation_applied=" + std::to_string(decision.probationApplied ? 1 : 0) + " probation_hits_required=" + std::to_string(decision.probationHitsRequired) + " observed_probation_hits=" + std::to_string(trace.observedProbationHits) + " churn_enabled=" + std::to_string(churnSnapshot.enabled ? 1 : 0) + " churn_active=" + std::to_string(churnSnapshot.active ? 1 : 0) + " churn_window_ms=" + std::to_string(churnSnapshot.windowMs) + " churn_window_samples=" + std::to_string(churnSnapshot.windowSamples) + " churn_one_hit_rate_pct=" + std::to_string(churnSnapshot.oneHitRatePct) + " churn_unique_keys=" + std::to_string(churnSnapshot.uniqueKeys) + " churn_enter_one_hit_rate_pct=" + std::to_string(churnSnapshot.enterOneHitRatePct) + " churn_exit_one_hit_rate_pct=" + std::to_string(churnSnapshot.exitOneHitRatePct) + " churn_probation_hit_bonus=" + std::to_string(churnSnapshot.probationHitBonus) + " keep_hot_ms=" + std::to_string(churnSnapshot.keepHotMs) + " readmit_cooldown_ms=" + std::to_string(churnSnapshot.readmitCooldownMs) + " ghost_hits_for_readmit=" + std::to_string(churnSnapshot.ghostHitsForReadmit) + " critical_current_frame=" + std::to_string(trace.criticalCurrentFrame ? 1 : 0) + " request_bytes=" + std::to_string(static_cast<unsigned long long>(trace.requestBytes)) + " max_durable_bytes=" + std::to_string(static_cast<unsigned long long>(decision.maxDurableBytes)) + " entry_digest=" + std::to_string(static_cast<unsigned long long>(trace.entryDigest)) + trace_device_context_fields(transaction) + " decision_reason=" + trace_or_unspecified(decision.reason) + " decision_reason_class=" + trace_reason_class_or_invalid(decision.reason) + " reason=" + trace_or_unspecified(trace.reason) + trace_reason_class_field_if_known("reason_class", trace.reason);
            JTRACE("MSADM", msg);
#endif
        }

        struct ProbationDecisionTrace {
            const char* commandName = nullptr;
            const char* reason = nullptr;
            std::uint64_t entryDigest = 0;
            std::uint32_t observedProbationHits = 0;
            std::uint32_t requiredProbationHits = 0;
            bool admitted = false;
        };

        void trace_probation_decision(
            const SubmissionTransaction& transaction,
            const ProbationDecisionTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }

            const std::string msg = trace_event_prefix("probation", transaction, trace.commandName) + " entry_digest=" + std::to_string(static_cast<unsigned long long>(trace.entryDigest)) + " observed_probation_hits=" + std::to_string(trace.observedProbationHits) + " required_probation_hits=" + std::to_string(trace.requiredProbationHits) + " admitted=" + std::to_string(trace.admitted ? 1 : 0) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(trace.reason) + " reason_class=" + trace_reason_class_or_invalid(trace.reason);
            JTRACE("MSPRB", msg);
#endif
        }
        ...
        void trace_reap_pass(
            ...
            const std::string msg = trace_event_prefix("reap_pass", transaction, commandName) + " reclaimed_bytes=" + std::to_string(static_cast<unsigned long long>(reclaimedBytes)) + " success=" + std::to_string(success ? 1 : 0) + trace_device_context_fields(transaction) + " reason=" + trace_or_unspecified(reason) + trace_reason_class_field_if_known("reason_class", reason);
            JTRACE("MSREAP", msg);
#endif
        }

        struct FragmentationRecoveryTrace {
            const char* commandName = nullptr;
        ...
        void trace_fragmentation_recovery(
            const SubmissionTransaction& transaction,
            const FragmentationRecoveryTrace& trace) {
#if JUICER_DIAGNOSTICS_COMPILED
            if (!JTRACE_ENABLED(2)) {
                return;
            }
        ...
        struct TelemetryQueryMutationTrace {
            const char* queryName = nullptr;
            const DeviceContextKey* key = nullptr;
            const char* reason = nullptr;
            std::uint64_t beforeThreadMutationTicket = 0;
            std::uint64_t afterThreadMutationTicket = 0;
            std::uint64_t beforeThreadMutationBeginCount = 0;
            std::uint64_t afterThreadMutationBeginCount = 0;
            std::uint32_t beforeThreadMutationDepth = 0;
            std::uint32_t afterThreadMutationDepth = 0;
        };
        ...
        QueryReadOnlyGuard::QueryReadOnlyGuard(
            ...
            : _queryName(queryName), _key(key), _before(take_query_read_only_snapshot()) {
        }
        ...
        QueryReadOnlyGuard::~QueryReadOnlyGuard() noexcept {
            ...
            telemetry_trace_query_mutation_violation(
                TelemetryQueryMutationTrace{
                    .queryName = _queryName,
                    .key = _key,
                    .reason = reason,
                    .beforeThreadMutationTicket = _before.threadMutationTicket,
                    .afterThreadMutationTicket = after.threadMutationTicket,
                    .beforeThreadMutationBeginCount = _before.threadMutationBeginCount,
                    .afterThreadMutationBeginCount = after.threadMutationBeginCount,
                    .beforeThreadMutationDepth = _before.threadMutationDepth,
                    .afterThreadMutationDepth = after.threadMutationDepth});
        }
        ...
        struct TelemetryTraceContext {
            ...
            std::uint64_t snapshotId = 0;
            std::uint32_t traceSchemaVersion = 0;
        };
        ...
        std::string telemetry_trace_txn_snapshot_schema_prefix(const TelemetryTraceContext& context) {
            return telemetry_trace_txn_snapshot_prefix(context) + " trace_schema=" + std::to_string(context.traceSchemaVersion);
        }
        ...
        struct TelemetryEventStageTrace {
            const char* eventName = nullptr;
            const char* stage = nullptr;
        };

        std::string telemetry_trace_event_stage_prefix(const TelemetryEventStageTrace& trace) {
            return telemetry_trace_event_prefix(trace.eventName) + " stage=" + trace_token_or(trace.stage, kTraceTokenUnknown);
        }

        struct TelemetryPhaseStageTrace {
            const char* phase = nullptr;
            const char* stage = nullptr;
        };

        std::string telemetry_trace_phase_stage_prefix(const TelemetryPhaseStageTrace& trace) {
            return std::string("phase=") + trace_token_or(trace.phase, kTraceTokenUnknown) + " stage=" + trace_token_or(trace.stage, kTraceTokenUnknown);
        }
        ...
        struct TelemetrySchemaMismatchTrace {
            std::uint64_t transactionId = 0;
            std::uint64_t snapshotId = 0;
            std::uint32_t observedTraceSchemaVersion = 0;
        };
        ...
        struct TelemetryKeyNormalizationTrace {
            TelemetryTraceContext context{};
            const KeyDigests* before = nullptr;
            const KeyDigests* after = nullptr;
        };
        ...
        struct TelemetryInvalidationTrace {
            TelemetryTraceContext context{};
        ...
        struct TelemetryDagEdgeTrace {
            TelemetryTraceContext context{};
            const char* fromNode = nullptr;
            const char* toNode = nullptr;
            const char* reason = nullptr;
            bool allowed = false;
        };
        ...
        struct TelemetryFrameSnapshotMismatchTrace {
            TelemetryTraceContext context{};
            std::uint64_t frameToken = 0;
            std::uint64_t expectedSnapshotId = 0;
            std::uint64_t observedSnapshotId = 0;
        };
        ...
        struct TelemetryStaleDecisionTrace {
            TelemetryTraceContext context{};
            const char* stage = nullptr;
            const StaleInput* input = nullptr;
            const StaleDecision* decision = nullptr;
        };
        ...
        struct TelemetryAcquireTrace {
            TelemetryTraceContext context{};
            const ResourcePlan* plan = nullptr;
            std::uint64_t acquireId = 0;
            AcquireStatus finalStatus = AcquireStatus::Error;
