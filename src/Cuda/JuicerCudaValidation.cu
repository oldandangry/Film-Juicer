...
namespace {
    ...
    const char* set_self_check_error(std::string& storage, const char* msg) {
        storage = msg ? msg : "(unknown)";
        return storage.c_str();
    }

    const char* set_self_check_error_cuda(std::string& storage, const char* prefix, cudaError_t err) {
